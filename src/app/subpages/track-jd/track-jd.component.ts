...
jobColumns = [
  ...
  fetchJds(): void {
    this.loading = true;
    this.jdService.getJobDescriptions().subscribe({
      next: (data) => {
        // Convert action strings into an array if needed
        this.jobDataSource = data.map(jd => ({
          ...jd,
           jrstatus: jd.status,
          actions: ['View JD', 'View Applicants']
        }));
        this.loading = false;
      },
      error: (err) => {
  ...
  handleActionClick(action: string, jd: any): void {
    if (action === 'View JD') {
      this.router.navigate(['../jd-details', jd.id], { relativeTo: this.route });
    } else if (action === 'View Applicants') {
      this.router.navigate(['../applicants', jd.id], { relativeTo: this.route });
    }
  }
  onViewJD = (row: any) => {
  this.router.navigate(['/recruiter/job-description/details']);
