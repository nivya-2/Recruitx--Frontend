...
export class InterviewPanelService {
  ...
  private baseUrl = 'https://recruitx20250704121717-fdb4cqaze6bsgcfr.canadacentral-01.azurewebsites.net/api/interviewpanel';
  ...
  getPanelMemberMeetingss(): Observable<InterviewMeeting[]> {
    ...
    // 1. Tell HttpClient to expect the ApiResponse that wraps our array.
    return this.http.get<ApiResponse<InterviewMeeting[]>>(`https://recruitx20250704121717-fdb4cqaze6bsgcfr.canadacentral-01.azurewebsites.net/api/InterviewPanel/panel-meetings?email=jessicabrown%40RecruitXexp.onmicrosoft.com&date=10-06-2025`).pipe(
      // 2. Use the map operator to extract and return only the 'data' property (the array).
      map(response => response.data)
    );
  }
